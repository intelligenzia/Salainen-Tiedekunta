...
export default function Index() {
  ...
  return (
    <>
      ...
      />
      <SafeAreaView className="flex-1 bg-white dark:bg-zinc-900" edges={isWeb ? ['top', 'bottom'] : ['bottom']}>
        <ScrollView className="flex-1">
          <View className="max-w-4xl mx-auto w-full">
            ...
            <View className="p-4 md:p-6 border-b border-zinc-200 dark:border-zinc-700">
            ...
            <View className="p-4 md:p-6 border-b border-zinc-200 dark:border-zinc-700">
              <Text className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-4 tracking-tight">
                ...
                <View className="gap-3">
                  {featuredCourses.map((course) => (
                    ...
                    >
                      <Link.Trigger>
                        <Pressable
                          className="border border-zinc-200 dark:border-zinc-700 rounded-lg p-4 active:bg-zinc-50 dark:active:bg-zinc-800"
                        >
                          <View className="flex-row justify-between items-start">
                            <View className="flex-1 pr-4">
                              ...
                              <Text className="text-zinc-900 dark:text-zinc-100 font-medium text-sm ">
                                {course.fields.name}
                              </Text>
                            </View>
            ...
            {showNavigationSection && (
              <View className="p-6 border-b border-zinc-200 dark:border-zinc-700">
                <Text className="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-4 tracking-tight">
                  Selaa opintoja
                </Text>
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href as any} asChild>
                    <Pressable className="border-b border-zinc-100 dark:border-zinc-800 py-4 active:bg-zinc-50 dark:active:bg-zinc-800">
                  ...
                  {facultyInfo.map((item) => (
                    <AccordionItem key={item.id} value={item.id}>
                      <AccordionTrigger className="px-4">
                        <Text className="text-zinc-900 dark:text-zinc-100 font-medium text-sm flex-1 text-left">
                          {item.title}
                        </Text>
                      </AccordionTrigger>
                      <AccordionContent>
                        <Text className="text-zinc-500 dark:text-zinc-400 text-sm leading-relaxed m-4">
                          {item.content}
                        </Text>
                      </AccordionContent>
                    </AccordionItem>
